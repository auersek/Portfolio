...
class Mnist_Loader:
    ...
    def vectorized_result(self, j):
        ...
        return e
class LLBIC:
    def __init__(self, no_of_instance=100, no_of_classes=10, no_of_laws=784, H=4, device=torch.device('cpu')):
        ...
        def convert_to_tensor(data):
            ...
            classes_tensor = torch.tensor(classes, dtype=torch.int64)  # Use int64 for class labels
        ...
        print(f"LLBIC initialized with feature matrix shape: {self.features_matrix.shape}")
    ...
    def _get_law(self, in_matrix):
        ...
        if not torch.allclose(in_matrix, in_matrix.T):
            print("Matrix is not symmetric")

        epsilon = 0.001
        if torch.all(in_matrix == 0):
            in_matrix += epsilon * torch.eye(in_matrix.size(0), device=self.device)
    ...
    def _extract_features_matrix(self):
        ...
        # Partition and average the law matrices by class
        for cls in range(self.no_of_classes):
            ...
            print(f"Class {cls} average shape: {class_average.shape}")
    ...
    def train(self, cleanup=False):
        ...
        # Check if all training instances are unique
    ...
    def _check_uniqueness(self, data_tensor):
        ...
        data_flat = data_tensor.view(data_tensor.size(0), -1)  # Flatten each instance
    ...
    def _multiply(self, stacked_embeddings):
        """ Multiply the embedded matrix with the feature matrices. """
        ...
        for cls in range(self.no_of_classes):
            P_cls = P[:, :, cls]
            result_cls = torch.matmul(P_cls.T, stacked_embeddings)
            result_list.append(result_cls)
        
        result_stack = torch.stack(result_list, dim=0)  # Shape: [10, 784, 3136]
        result_mean = torch.mean(result_stack, dim=1)   # Average across pixel dimension: Shape [10, 3136]
        return result_mean
    ...
    def classify(self, instance, z):
        ...
        # Create embeddings for each pixel
        for t in range(784):
            S = self._square_embed_test(instance, t)  # Use 0 as placeholder for center_index
            embeddings.append(S)
        stacked_embeddings = torch.cat(embeddings, dim=1)  # Concatenate along dim=1

        print(f"Stacked embeddings shape: {stacked_embeddings.shape}")

        results = self._multiply(stacked_embeddings)
        
        averaged_results = torch.mean(results, dim=1)  # Average across pixel dimension
        print(f"Averaged results: {averaged_results}")

        # Find the class with the minimum average score
        _, predicted_class = torch.min(averaged_results, dim=0)
    ...
    def evaluate(self):
        ...
        total_samples = len(self.test_inputs)

        # Iterate over each test sample
        for i in range(total_samples):
            test_input = self.test_inputs[i].unsqueeze(0)  # Add batch dimension
            actual_label = self.test_classes[i].item()  # Use the actual label value directly
            print(f"Test input shape: {test_input.shape}")
            print(f"Actual label: {actual_label}")

            # Flatten the input to match expected shape
            predicted_class = self.classify(i, test_input.numpy().flatten())  
...
if __name__ == "__main__":
    ...
    llbic = LLBIC(no_of_instance=100, no_of_classes=10, no_of_laws=784, H=4, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    llbic.train()
    llbic.evaluate()
