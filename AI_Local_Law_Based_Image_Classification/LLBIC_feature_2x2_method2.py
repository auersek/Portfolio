...
class LLBIC:
    def __init__(self, no_of_instance=50, no_of_classes=10, no_of_laws=196, H=4, device=torch.device('cpu')):
    ...
    def _update_feature_matrix(self, instance_index):
        ...
        current_min_avg = current_result.mean().item()  # Calculate average for the current class
        print(f"Current min average for class {instance_class}: {current_min_avg}")
        ...
        for cls in range(self.no_of_classes):
            ...
            for feature_matrix in self.features_matrix[cls]:
                ...
                min_avg_other_class = result.mean().item()
                print(f"Minimum average for class {cls}: {min_avg_other_class}")

                # Check if the minimum of the averaged result from any other class is smaller
                if min_avg_other_class < current_min_avg:
    ...
    def _extract_features_matrix(self):
        ...
        normalize_for_class[0] += 0.9
        normalize_for_class[1] -= 0.5
        # normalize_for_class[2] += 0.1
        normalize_for_class[3] += 0.1
        # normalize_for_class[5] -= 0.01
        normalize_for_class[9] -= 0.01
        normalize_for_class[8] += 4
...
if __name__ == "__main__":
    ...
    llbic = LLBIC(no_of_instance=50, no_of_classes=10, no_of_laws=196, H=4,
