...
class LLBIC:
    def __init__(self, no_of_instance=1000, no_of_classes=10, no_of_laws=49, H=4, device=torch.device('cpu')):
        ...
        for cls in range(self.no_of_classes):
            ...
            for col in range(num_blocks):
                ...
                if start_row >= end_row:
                    continue  # We want 2 rows for the block

                # Ensure P_cls_block is a (2, N) tensor (2xN block for the current class)
                P_cls_block = P_cls[:, col].view(4, -1)  # Ensure the view is compatible with the dimensions
