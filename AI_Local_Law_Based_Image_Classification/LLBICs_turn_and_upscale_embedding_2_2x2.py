...
class Mnist_Loader:
    ...
    def _prepare_data(self, data, label_vectorization=True):
        """Helper function to reshape, upscale images, apply the 0.2 filter, and vectorize labels."""
        images = [self.upscale_image(self.apply_threshold(np.reshape(x, (28, 28)))) for x in data[0]]  # Apply 0.2 filter & upscale
        print("Images upscaled and 0.2 filter applied")
    ...
    def upscale_image(self, image, scale_factor=2):
        """Upscale the image by a specified scale factor."""
        new_size = (image.shape[1] * scale_factor, image.shape[0] * scale_factor)  # (width, height)
        upscaled_image = cv2.resize(image, new_size, interpolation=cv2.INTER_LINEAR)  # Upscale using linear interpolation
...
class LLBIC:
    def __init__(self, no_of_instance=50000, no_of_classes=10, no_of_laws=784, H=2, device=torch.device('cpu')):
        ...
        def convert_to_tensor(images, labels):
            ...
            # Reshape images based on the new size (56 * 56 = 3136)
            inputs_tensor = torch.tensor(np.array(images).reshape(len(images), 3136), dtype=torch.float32)
        ...
        self.H = H
        self.features_matrix = torch.zeros((self.H, 784, self.no_of_classes), device=self.device)  # Now 784 blocks after upscaling



    def _square_embed(self, instance_index, block_index):
        """Extract a 2x2 block from the upscaled 56x56 image and compute its symmetric matrix."""
        block_row = block_index // 28  # 28 blocks per row after upscaling
        block_col = block_index % 28   # 28 blocks per column after upscaling
        ...
        for i in range(2):
            for j in range(2):
                row = start_row + i
                col = start_col + j
                if row < 56 and col < 56:  # Ensure we stay within 56x56 boundary
                    calculated_index = row * 56 + col
    ...
    def _square_embed_test(self, instance_index, block_index):
        """Extract a 2x2 block from the upscaled 56x56 test image and compute its symmetric matrix."""
        block_row = block_index // 28  # 28 blocks per row after upscaling
        block_col = block_index % 28   # 28 blocks per column after upscaling
    ...
    def _extract_features_matrix(self):
        ...
        all_laws_tensor = torch.zeros((self.H, 784, self.no_of_instance), device=self.device)  # Adjust for 784 blocks (28x28 grid)

        # Extract features for each instance
        for i in range(self.no_of_instance):
            print(f"Extracting isntance: {i}")
            for t in range(784):  # Now iterate over 784 blocks
    ...
    def classify(self, instance, z):
        ...
        embeddings_tensor = torch.stack(embeddings)  # Shape: (784, 2, 2)
    ...
    def classify_test(self, z):
        ...
        for instance in range(self.test_inputs.size(0)):
            print(f"Classifying isntance: {instance}")
...
if __name__ == "__main__":
    ...
    llbic = LLBIC(no_of_instance=50000, no_of_classes=10, no_of_laws=784, H=2, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    llbic.train()
    llbic.evaluate()
