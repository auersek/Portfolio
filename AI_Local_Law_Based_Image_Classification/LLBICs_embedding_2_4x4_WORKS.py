"""
I am working with the MNIST database where each image represents a digit and is 28by28 pixels, it also has the corresponding class for each of the instances in the second dimension, which is 1by10 matrix where the corresponding class has the highest value and the others 0. A law matrix for each instance should be calculated, creating a 4by49 matrix of laws. For each training instance after it is calculated it should be added to the tensor of laws, which would have dimensions 4*49*n where n is the number of instances. After all of the training instance law matrices are calculated, the law tensor should be partitioned so that each digit 0-9 have its own corresponding matrices grouped together. This is then averaged to create the features matrix, which should be a 4by49by10 tensor, where 4 is the length of the eigenvector, 49 is the number of 4 by 4 sqaures on the image, and  10 is the number of classes (digits 0-9). This feature matrix is to be used for classification. To classify each test instance, embedd it so that it is a 4by4 matrix as before but concatenate all of these to create an embedded matrix of size 49 by 4. Multiply this embedded matrix with each the feature matrices, and average the product into a single number and save this. The scale closest to zero on average should be the class that the test instance belongs to. 
"""

wignerrobot@LBT:~/Desktop/LLFT-master-master$ /bin/python /home/wignerrobot/Desktop/LLFT-master-master/LLBICworking.py
...
class LLBIC:
        ...
        for i in range(4):
            for j in range(4):
                ...
                if row < 28 and col < 28:
                    ...
                    four_by_four_box[i, j] = self.train_inputs[instance_index, calculated_index]

        symmetric_matrix = torch.matmul(four_by_four_box, four_by_four_box.T)
        return symmetric_matrix

    def _four_by_four_embed_test(self, instance_index, block_index):
        """ Extract a 4x4 block from the image (test data) and compute its symmetric matrix. """
        block_row = block_index // 7
        block_col = block_index % 7
        ...
        for i in range(4):
            for j in range(4):
                ...
                if row < 28 and col < 28:
                    calculated_index = row * 28 + col
                    four_by_four_box[i, j] = self.test_inputs[instance_index, calculated_index]
    ...
    def _extract_features_matrix(self):
        ...
        for i in range(self.no_of_instance):
            for t in range(49):
                S = self._four_by_four_embed(i, t)
                law = self._get_law(S)
        ...
        for cls in range(self.no_of_classes):
          ...
          #   print(f"Class {cls} average: {class_average}")
    ...
    def _multiply(self, stacked_embeddings):
        ...
        # print(f"stacked_embeddings shape: {stacked_embeddings.shape}")
        ...
        for cls in range(self.no_of_classes):
            ...
            for col in range(num_blocks):
                ...
                # print(f"Processing class {cls}, block column {col}")
                # print(f"P_cls_block shape: {P_cls_block.shape}")
                # print(f"stacked_embeddings[start_row:end_row, :] shape: {stacked_embeddings[start_row:end_row, :].shape}")
        ...
        # print(f"result_mean shape: {result_mean.shape}")
    ...
    def classify(self, instance, z):
        ...
        z = z.to(self.device)
        embeddings = [self._four_by_four_embed_test(instance, center_index) for center_index in range(49)]
