...
class LLBIC:
    def __init__(self, no_of_instance=50000, no_of_classes=10, no_of_laws=196, H=2, device=torch.device('cpu')):
    ...
    def _extract_features_matrix(self):
        """Compute and store the features matrix with per-block class-based normalization."""
        ...
        for cls in range(self.no_of_classes):
            # Multiply each column (block) of the feature matrix by the corresponding row in normalize_for_pixels
            for block_index in range(196):
                self.features_matrix[:, block_index, cls] *= normalize_for_pixels[block_index, cls]
    ...
    def _multiply(self, stacked_embeddings):
        ...
        for cls in range(self.no_of_classes):
            ...
            for col in range(num_blocks):
                ...
                P_cls_block = P_cls[:, col].view(2, -1)
    ...
    def classify(self, instance, z):
        ...
        if second_lowest_score / lowest_score >= 1.04:
...
if __name__ == "__main__":
    ...
    llbic = LLBIC(no_of_instance=50000, no_of_classes=10, no_of_laws=196, H=2, device=torch.device('cuda' if torch.cuda.is_available() else 'cpu'))
    llbic.train()
    llbic.evaluate()
