...
static float t_error = 0;
static float prev_te = 0;
static float Pto     = 0;
static float Dto     = 0;
static float Ito     = 0;


const float kp = 2200, kd = 64, ki = 1;
...
std::vector<std::pair<int,int>> aStarPath(
    ...
    while (!openSet.empty()) {
        ...
        for (auto tmp2 : getNeighbors(current.x, current.y, gridRef)) {
            int nx = tmp2.first;
            int ny = tmp2.second;
            if (closedSet.count({nx, ny})) continue;
            float tentativeG = current.g + 1.0;
            int key = hash(nx, ny);
...
void setco() {
    ...
    if (xdistance != 0) {
        ...
        if (WheelPos < xdistance && errorX >= 0.11) {
            set_speed = -15;
        }
        else if (WheelPos < xdistance && errorX <= 0.15) {
    ...
    if (cmd[0] == 'x') {
        ...
        switch (cmd[0]) {
            ...
            case 'q':
                ...
                isTurningAnti = false;
                isAutonomous = false;
                CurXCoord = CurYCoord = 0.0;
                PrevXDistance = PrevYDistance = 0.0;
...
void handlePose() {
    ...
    auto tmp1 = worldToGrid(CurXCoord, CurYCoord);
    int robotGX = tmp1.first;
    int robotGY = tmp1.second;
...
void handleGoto() {
    ...
    auto tmp3 = worldToGrid(CurXCoord, CurYCoord);
    int startGX = tmp3.first;
    int startGY = tmp3.second;
...
void setup() {
    ...
    PrevWheelPos = 0.0;
    prev_theta_n = prev_speed = prevspin = 0.0;
...
void loop() {
    ...
    if (now > loopTimer) {
        ...
        accelAngle = (a.acceleration.z / 9.67) - 0.019;
        spinAngle  = (g.gyro.roll) + 0.031;
        ...
        s_error         = set_speed - current_speed;
        Ps              = s_error * sp;
        Ds              = -((s_error - prev_speed) / 0.005) * sd;
        Is              = (s_error + prev_speed) * si * 0.005;
        reference       = Ps + Ds + Is - 0.004;

        theta_n         = (1 - c) * accelAngle + c * (((gyroAngle - 0.02) * 0.005) + prev_theta_n);

        err      = reference - theta_n;
        prev_err = reference - prev_theta_n;
        Pout     = err * kp;
        Dout     = -((err - prev_err) / 0.005) * kd;
        Iout     = (err + prev_err) * ki * 0.005;
        prev_theta_n   = Pout + Dout + Iout;

        t_error = turn_reference - spinComp;
        prev_te = turn_reference - prevspin;
        Pto     = t_error * tp;
        Dto     = ((t_error - prev_te) / 0.005) * td;
        Ito     = (t_error + prev_te) * ti * 0.005;
    ...
    if (now > printTimer) {
        ...
        Serial.print(", TURN REFERENCE:"); Serial.print(turn_reference, 2);
        Serial.print(", spinRad:"); Serial.println(spinRad, 2);
    }

    if (now > odomToGridTimer) {
        odomToGridTimer = now + 100;
        auto tmp4 = worldToGrid(CurXCoord, CurYCoord);
        int newGX = tmp4.first;
        int newGY = tmp4.second;
    ...
    if (pathActive && pathIndex < path.size()) {
        auto tmp6   = path[pathIndex];
        int tx = tmp6.first;
        int ty = tmp6.second;
        auto tmp7 = worldToGrid(CurXCoord, CurYCoord);
        int rgx = tmp7.first;
        int rgy = tmp7.second;
